...
### New Features
- **长期记忆自动记录开关** — 设置页新增“自动记录长期记忆”；关闭后网页聊天、语音聊天和后端自动学习均停止写入，已有记忆与手动管理保持可用

## 2026-03-10

### New Features
- **记忆时间线** — 设置 → 长期记忆面板新增时钟按钮，展开 Git log 风格的 2D 垂直时间线，展示所有记忆变更历史（新增/更新/删除/合并/撤销/整合），按日期分组，支持清空日志（不影响实际记忆）
- **记忆变更审计日志** — 后端自动记录 auto-learn / undo / reflect 操作到 `data/memory-log.jsonl`，5000 行自动截断，互斥锁保护并发安全
- **设置面板 footer 固定** — 保存/恢复按钮固定在面板底部，不再随内容滚动漏出

### Improvements
- **默认 AI 名称** — 输入框 placeholder 和设置面板默认显示从"4o"改为"AI"
